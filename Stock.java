...
public class Stock 
{
	...
	static void displayAscending(StockHolding stocks[])
	{
		//selection sort
		for(int i=0; i<stocks.length; i++)
		{
			...
			for(int j=i+1; j<stocks.length; j++)
			{
				...
				if((stocks[j].companyName.compareTo(stocks[i].companyName)) < 0)
				{
					temp = stocks[j];
					stocks[j]=stocks[i];
					stocks[i]=temp;
	...
	static void displayDescending(ForeignStockHolding stocks[])
	{
		//selection sort
		for(int i=0; i<stocks.length; i++)
		{
	...
	static void displayAscending(ForeignStockHolding stocks[])
	{
		//selection sort
		for(int i=0; i<stocks.length; i++)
		{
	...
	static void displayMinValue (ForeignStockHolding stocks[])
	{
		//temp object of ForeignStockHolding to store min value stock
		ForeignStockHolding temp =stocks[0];
		
		//traversing the array to find the min value stock
		for(int i=1; i<stocks.length; i++)
		{
			if(temp.valueInDollars() > stocks[i].valueInDollars())
			{
				temp = stocks[i];
			}
		}
	...
	//function to display max value stock
	static void displayMaxValue (ForeignStockHolding stocks[])
	{
	...
	//function to Display the most profitable stock
	static void displayMaxProfitableStock(ForeignStockHolding stocks[])
	{
		...
		for(int i=1; i<stocks.length; i++)
		{
			if((temp.valueInDollars()-temp.costInDollars()) < (stocks[i].valueInDollars()-stocks[i].costInDollars()))
			{
				temp = stocks[i];
			}
		}
	...
	//function to Display the least profitable stock
	static void displayMinProfitableStock(ForeignStockHolding stocks[])
	{
	...
	static void displayInSortedValueOrder (ForeignStockHolding stocks[])
	{
		//selection sort
		for(int i=0; i<stocks.length; i++)
		{
			...
			{
				//if value at index j is lexicographically less than that at i swap them
				if(stocks[j].valueInDollars() < stocks[i].valueInDollars())
				{
					temp = stocks[j];
					stocks[j]=stocks[i];
					stocks[i]=temp;
	...
	public static void main (String[] args)
	{
		...
		stockHoldingArray[0] = new StockHolding((float)4.30, (float)5.50, (int)60, "XYZ limited");
		stockHoldingArray[1] = new StockHolding((float)2.19, (float)13.56, (int)100, "ABC Pvt Limited");
		stockHoldingArray[2] = new StockHolding((float)5.10, (float)4.51, (int)200, "PVR Limited");
		...
		foreignStockHoldingArray[0] = new ForeignStockHolding((float)4.30, (float)5.50, (int)60, "XYZ limited", (float)1.94);
		foreignStockHoldingArray[1] = new ForeignStockHolding((float)2.19, (float)13.56, (int)100, "ABC Pvt Limited", (float)0.10);
		foreignStockHoldingArray[2] = new ForeignStockHolding((float)5.10, (float)4.51, (int)200, "PVR Limited", (float)2.25);
